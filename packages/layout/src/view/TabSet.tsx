...
export class TabSet extends React.Component<ITabSetProps, any> {
    ...
    componentWillReceiveProps(_nextProps: ITabSetProps) {
        this.showToolbar = true;
        this.showOverflow = false;
        this.recalcVisibleTabs = true;
    ...
    render() {
        ...
        if (this.showToolbar === true) {
            if (this.props.node.isEnableMaximize()) {
                if (node.isMaximized()) {
                    buttons.push(
                        <button
                            key="min"
                            className={"flexlayout-tab-toolbar-button flexlayout-tab-toolbar-button-min"}
                    ...
                    buttons.push(
                        <button
                            key="max"
                            className={"flexlayout-tab-toolbar-button flexlayout-tab-toolbar-button-max"}
                            onClick={this.onMaximizeToggle}
                        >
                            <Icon icon={"maximize"} />
                        </button>);
            ...
            buttons.push(
                <button
                    key="close"
                    className={"flexlayout-tab-toolbar-button flexlayout-tab-toolbar-button-close"}
                    onClick={this.onMaximizeToggle}
                >
                    <Icon icon={"cross"} />
                </button>);
            toolbar = <div key="toolbar" ref={ref => this.toolbarRef = (ref === null) ? undefined : ref} className={cm("flexlayout-tab-toolbar")}
        ...
        if (this.showOverflow === true) {
            tabs.push(<button key="overflowbutton" ref={ref => this.overflowbuttonRef = (ref === null) ? undefined : ref} className={cm("flexlayout-tab-button-overflow")}
        ...
        let tabStripClasses = cm("flexlayout-tab-header-outer");
        if (this.props.node.getClassNameTabStrip() !== undefined) {
            tabStripClasses += " " + this.props.node.getClassNameTabStrip();
        }
        if (node.isActive() && !showHeader) {
            tabStripClasses += " " + cm("flexlayout-tabset-selected");
        }

        if (node.isMaximized() && !showHeader) {
            tabStripClasses += " " + cm("flexlayout-tabset-maximized");
        }

        if (showHeader) {
            let tabHeaderClasses = cm("flexlayout-tabset-header");
            if (node.isActive()) {
                tabHeaderClasses += " " + cm("flexlayout-tabset-selected");
            }
            if (node.isMaximized()) {
                tabHeaderClasses += " " + cm("flexlayout-tabset-maximized");
            }
            ...
            tabStrip = <div className={tabStripClasses}
                ...
                <div ref={ref => this.headerRef = (ref === null) ? undefined : ref} className={cm("flexlayout-tab-header-inner")}>
                    {tabs}
                </div>
            </div>;
        }
        ...
        return <div style={style} className={cm("flexlayout-tabset")}>
            {header}
            {tabStrip}
        </div>;
    }
