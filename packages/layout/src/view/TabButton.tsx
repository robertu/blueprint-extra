...
export class TabButton extends React.Component<ITabButtonProps, any> {
    ...
    render() {
        ...
        let classNames = cm("flexlayout-tab-button");
        const node = this.props.node;
        const parentNode = this.props.node.getParent() as TabSetNode;
        const children = parentNode.getChildren();
        if (this.props.selected && children.length > 1) {
            classNames += " " + cm("flexlayout-tab-button-selected");
        }
        else {
            classNames += " " + cm("flexlayout-tab-button-unselected");
        }
        ...
        let content = <div ref={ref => this.contentRef = (ref === null) ? undefined : ref} className={cm("flexlayout-tab-button-content")}>{renderState.content}</div>;
        // const leading = <div className={cm("flexlayout-tab-button-leading")}>{renderState.leading}<DropdownMenu /></div>;
        ...
        if (this.state.editing) {
            ...
            content = <input style={contentStyle}
                ...
                className={cm("flexlayout-tab-button-textbox")}
                type="text"
                autoFocus={true}
        ...
        if (this.props.node.isEnableClose()) {
            closeButton = <div className={cm("flexlayout-tab-button-trailing")}
        ...
        return (
            ...
            >
                <div className="dropdown-menu-container" onMouseDown={this.onCloseMouseDown} onTouchStart={this.onCloseMouseDown}>
