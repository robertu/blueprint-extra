...
export class Tab extends React.Component<ITabProps, any> {
    ...
    render() {
        ...
        const flexlayoutBorderInnerBottom : HTMLElement | null = document.querySelector(".flexlayout-border-inner-bottom");
        const flexlayoutTabBorderBottom : HTMLElement | null = document.querySelector(".flexlayout-tab-border-bottom");
        const style: JSMap<any> = node._styleWithPosition({
            display: this.props.selected ? "block" : "none"
        });
        if (nodeId === "border-bottom") {
            style.borderRadius = "3px 3px 0px 0px";
        }
        if (nodeId === "border-left") {
            style.borderRadius = "0px 3px 3px 0px";
        }
        if (nodeId === "border-right") {
        ...
        return <div className={cm(`flexlayout-tab flexlayout-tab-${nodeId} ${classes}`)}
