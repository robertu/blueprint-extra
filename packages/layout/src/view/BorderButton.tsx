...
export class BorderButton extends React.Component<IBorderButtonProps, any> {
    ...
    render() {
        ...
        let classNames = cm("flexlayout-border-button") + " " +
            cm("flexlayout-border-button-" + this.props.border);
        const node = this.props.node;

        if (this.props.selected) {
            classNames += " " + cm("flexlayout-border-button-selected");
        }
        else {
            classNames += " " + cm("flexlayout-border-button-unselected");
        }
        ...
        const content = <div ref={ref => this.contentsRef = (ref === null) ? undefined : ref} className={cm("flexlayout-border-button-content")}>{renderState.content}</div>;
        const leading = <div className={cm("flexlayout-border-button-leading")}>{renderState.leading}</div>;


        let closeButton;
        if (this.props.node.isEnableClose()) {
            closeButton = <div className={cm("flexlayout-border-button-trailing")}
                onMouseDown={this.onCloseMouseDown}
                onClick={this.onClose}
                onTouchStart={this.onCloseMouseDown}
                style={parent.getClassName() === "border-right" ? {marginLeft: "10px", marginRight: "0px"} : {marginLeft: "0px", marginRight: "10px"}}
            ><Icon icon="cross" /></div>;
        }
        return <div ref={ref => this.selfRef = (ref === null) ? undefined : ref}
            ...
            {leading}
            {
                parent.getClassName() !== "border-right"
                &&
                closeButton
            }
            {content}
