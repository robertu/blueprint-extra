...
class PopupMenu extends React.Component<IPopupMenuProps, any> {
  ...
  static show(triggerElement: Element,
    ...
    elm.className = classNameMapper("flexlayout-popup-menu-container");
  ...
  render() {
    const items = this.props.items.map(item => <div key={item.index} className={this.props.classNameMapper("flexlayout-popup-menu-item")}
      onClick={this.onItemClick.bind(this, item)}>{item.name}</div>);

    return <div className={this.props.classNameMapper("flexlayout-popup_menu")}>
      {items}
    </div>;
  }
}
