...
class BorderNode extends Node implements IDropTarget {
  ...
  constructor(location: DockLocation, json: any, model: Model) {
    ...
    this._drawChildren = [];
    this._attributes.id = `border-${location.getName()}`;
  ...
  canDrop(dragNode: (Node & IDraggable), x: number, y: number): DropInfo | undefined {
    ...
    if (this._tabHeaderRect!.contains(x, y)) {
        ...
        if (this._children.length > 0) {
          ...
          for (let i = 0; i < this._children.length; i++) {
            ...
            if (x >= pos && x < childCenter) {
              ...
              dropInfo = new DropInfo(this, outlineRect, dockLocation, i, "flexlayout-outline-rect");
              break;
            }
            pos = childCenter;
          }
          if (dropInfo == null) {
            ...
            dropInfo = new DropInfo(this, outlineRect, dockLocation, this._children.length, "flexlayout-outline-rect");
          }
        } else {
          const outlineRect = new Rect(this._tabHeaderRect!.x + 1, this._tabHeaderRect!.y + 2, 3, 18);
          dropInfo = new DropInfo(this, outlineRect, dockLocation, 0, "flexlayout-outline-rect");
      ...
      dropInfo = new DropInfo(this, outlineRect!, dockLocation, -1, "flexlayout-outline-rect");
      if (!dragNode._canDockInto(dragNode, dropInfo)) {
        return undefined;
      }
    }
